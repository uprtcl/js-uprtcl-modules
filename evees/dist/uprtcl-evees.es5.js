...
import { gql, ApolloClient, ApolloLink } from 'apollo-boost';
import { injectable, inject, multiInject } from 'inversify';
import { CortexModule, PatternRecognizer, Pattern, PatternsModule } from '@uprtcl/cortex';
import { defaultCidConfig, loadEntity, DiscoveryModule, CASModule, EntityCache } from '@uprtcl/multiplatform';
import { Logger, moduleConnect, MicroModule, i18nextModule } from '@uprtcl/micro-orchestrator';
...
import { UprtclPopper, UprtclDialog, CommonUIModule } from '@uprtcl/common-ui';
import { LitElement, html, css, property, query, TemplateResult, internalProperty } from 'lit-element';
...
async function hashObject(object, config = defaultCidConfig) {
...
async function deriveEntity(object, config = defaultCidConfig) {
...
async function deriveSecured(object, config = defaultCidConfig) {
...
class EveesHelpers {
    ...
    static async getPerspectiveRemoteId(client, perspectiveId) {
        const perspective = await loadEntity(client, perspectiveId);
        if (!perspective)
    ...
    static async getCommitData(client, commitId) {
        ...
        const data = await loadEntity(client, dataId);
        if (!data)
    ...
    static async getCommitDataId(client, commitId) {
        const commit = await loadEntity(client, commitId);
        if (!commit)
            throw new Error('commit not found');
        return commit.object.payload.dataId;
    }
    static async getData(client, recognizer, uref) {
        const entity = await loadEntity(client, uref);
        if (!entity)
            return undefined;
...
let Evees = class Evees {
    ...
    async forkPerspective(perspectiveId, workspace, remote, parentId, name) {
        ...
        const refPerspective = await loadEntity(this.client, perspectiveId);
        if (!refPerspective)
...
Evees = __decorate([
    injectable(),
    __param(0, inject(CortexModule.bindings.Recognizer)),
    __param(1, multiInject(EveesBindings.EveesRemote)),
    __param(2, inject(ApolloClientModule.bindings.Client)),
    __param(3, inject(EveesBindings.Config)),
    __metadata("design:paramtypes", [PatternRecognizer, Array, ApolloClient, Object])
], Evees);

const propertyOrder = ['creatorId', 'path', 'remote', 'timestamp'];
class PerspectivePattern extends Pattern {
    constructor() {
        super(...arguments);
...
let PerspectiveLinks = class PerspectiveLinks {
    constructor(client) {
        this.client = client;
...
PerspectiveLinks = __decorate([
    injectable(),
    __param(0, inject(ApolloClientModule.bindings.Client)),
    __metadata("design:paramtypes", [ApolloClient])
], PerspectiveLinks);

const propertyOrder$1 = ['creatorsIds', 'timestamp', 'message', 'parentsIds', 'dataId'];
class CommitPattern extends Pattern {
    constructor() {
        super(...arguments);
...
let CommitLinked = class CommitLinked {
    constructor() {
...
CommitLinked = __decorate([
    injectable()
], CommitLinked);

class CommitHistory extends moduleConnect(LitElement) {
    async firstUpdated() {
        this.loadCommitHistory();
    }
    async loadCommitHistory() {
        this.commitHistory = undefined;
        const apolloClient = this.request(ApolloClientModule.bindings.Client);
...
class EveesWorkspace {
    ...
    buildWorkspace(client) {
        ...
        const workspace = new ApolloClient({
            cache: cloneDeep$1(client.cache),
...
const getContextPerspectives = async (context, container) => {
    ...
    const knownSources = container.get(DiscoveryModule.bindings.LocalKnownSources);
...
const eveesResolvers = {
    ...
    Mutation: {
        async updatePerspectiveHead(parent, { perspectiveId, headId, name }, { container }) {
            const evees = container.get(EveesBindings.Evees);
            const multiSource = container.get(DiscoveryModule.bindings.MultiSourceService);
        ...
        async createEntity(_, { id, object, casID }, { container }) {
            const stores = container.getAll(CASModule.bindings.CASStore);
            ...
            const entityCache = container.get(DiscoveryModule.bindings.EntityCache);
        ...
        async forkPerspective(_, { perspectiveId, remote, parentId, name }, { container }) {
            ...
            const perspective = await loadEntity(client, newPerspectiveId);
            if (!perspective)
                throw new Error('perspective not found');
            return {
                id: newPerspectiveId,
...
class EveesPerspectivesList extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-PERSPECTIVES-LIST');
...
class EveesInfoPopper extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-INFO-POPPER');
...
class EveesDiff extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
...
class EveesInfoBase extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-INFO');
    ...
    async load() {
        ...
        if (this.entityType === EveesBindings.PerspectiveType) {
            ...
            const head = headId !== undefined ? await loadEntity(this.client, headId) : undefined;
        ...
        if (this.entityType === EveesBindings.CommitType) {
            const head = await loadEntity(this.client, this.uref);
    ...
    async updatesDialog(workspace, options, message = html ``) {
...
__decorate([
    ...
    __metadata("design:type", UprtclDialog)
...
class EveesInfoPage extends EveesInfoBase {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-INFO-PAGE');
...
let SimpleMergeStrategy = class SimpleMergeStrategy {
...
SimpleMergeStrategy = __decorate([
    injectable(),
    __param(0, inject(EveesBindings.Evees)),
    __param(1, inject(CortexModule.bindings.Recognizer)),
    __param(2, inject(ApolloClientModule.bindings.Client)),
    __param(3, inject(DiscoveryModule.bindings.EntityCache)),
    __metadata("design:paramtypes", [Evees,
        PatternRecognizer,
        ApolloClient,
        EntityCache])
], SimpleMergeStrategy);

let RecursiveContextMergeStrategy = class RecursiveContextMergeStrategy extends SimpleMergeStrategy {
    constructor() {
        super(...arguments);
    ...
    async getPerspectiveContext(perspectiveId) {
        ...
        if (this.allPerspectives[perspectiveId]) {
            ...
            const secured = await loadEntity(this.client, perspectiveId);
            if (!secured)
...
RecursiveContextMergeStrategy = __decorate([
    injectable()
...
class EveesAuthor extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-AUTHOR');
        this.showName = false;
        this.short = false;
        this.loading = true;
        this.profile = {};
...
class ProposalsList extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-PERSPECTIVES-LIST');
...
class EveesProposalDiff extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-PROPOSAL-DIFF');
        this.summary = false;
        this.loading = true;
    }
    async firstUpdated() {
...
class EveesLoginWidget extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-LOGIN');
        this.loading = true;
...
class EveesProposalRow extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-PROPOSAL-ROW');
        this.loading = true;
    ...
    async load() {
        ...
        const fromPerspective = this.proposal.fromPerspectiveId
            ? await loadEntity(this.client, this.proposal.fromPerspectiveId)
            : undefined;
...
class EveesInfoUserBased extends EveesInfoBase {
    constructor() {
        super(...arguments);
        this.logger = new Logger('EVEES-INFO-UserBased');
        this.showProposals = false;
    ...
    async load() {
        ...
        const first = await loadEntity(this.client, this.firstRef);
        if (!first)
            throw new Error(`first perspective ${this.firstRef}`);
        const perspectiveIds = await this.getContextPerspectives(this.firstRef);
        const perspectives = (await Promise.all(perspectiveIds.map(perspectiveId => loadEntity(this.client, perspectiveId))));
        if (!this.defaultRemote)
        ...
        const nowPerspective = await loadEntity(this.client, this.uref);
        if (!nowPerspective)
...
class EveesContentModule extends MicroModule {
    constructor(stores = []) {
        super();
        this.stores = stores;
    ...
    get submodules() {
        return [new CASModule(this.stores.filter((store) => store.casID))];
    }
}

class EveesDB extends Dexie {
    constructor() {
        super('uprtcl');
...
class EveesPerspectiveIcon extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.loading = true;
    }
    ...
    async load() {
        this.loading = true;
        const perspective = await loadEntity(this.client, this.perspectiveId);
        if (!perspective)
...
export { CONTENT_UPDATED_TAG, CREATE_ENTITY, CREATE_PERSPECTIVE, CREATE_PROPOSAL, CommitHistory, CommitLinked, CommitPattern, ContentUpdatedEvent, DEFAULT_COLOR, Evees, EveesBindings, EveesContentModule, EveesDiff, EveesDraftsLocal, EveesHelpers, EveesInfoBase, EveesInfoPage, EveesInfoPopper, EveesInfoUserBased, EveesModule, EveesPerspectiveIcon, EveesPerspectivesList, EveesWorkspace, PROPOSAL_CREATED_TAG, PerspectiveLinks, PerspectivePattern, ProposalsList, RecursiveContextMergeStrategy, SimpleMergeStrategy, SpliceChildrenEvent, UPDATE_HEAD, UpdateContentEvent, deriveEntity, deriveSecured, eveeColor, extractSignedEntity, hashObject, isAncestorOf, mergeResult, mergeStrings, prettyAddress, signObject };
//# sourceMappingURL=uprtcl-evees.es5.js.map
