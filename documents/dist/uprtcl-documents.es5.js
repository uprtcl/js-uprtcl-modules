...
import { Pattern, recognizeEntity, CortexModule, PatternsModule } from '@uprtcl/cortex';
import { mergeResult, EveesModule, EveesDraftsLocal, EveesHelpers, hashObject, deriveSecured, CREATE_ENTITY, UPDATE_HEAD, ContentUpdatedEvent, eveeColor, EveesWorkspace, EveesContentModule } from '@uprtcl/evees';
import { ApolloClientModule, GraphQlSchemaModule } from '@uprtcl/graphql';
import { Logger, moduleConnect, i18nextModule } from '@uprtcl/micro-orchestrator';
...
class TextNodePattern extends Pattern {
    constructor() {
        super(...arguments);
        this.type = DocumentsBindings.TextNodeType;
    }
    recognize(entity) {
        return (recognizeEntity(entity) &&
            propertyOrder.every((p) => entity.object.hasOwnProperty(p)));
    }
}
let TextNodeCommon = class TextNodeCommon {
    constructor() {
        ...
        this.merge = (originalNode) => async (modifications, mergeStrategy, workspace, config) => {
            ...
            const resultType = mergeResult(originalNode.object.type, modifications.map((data) => data.object.type));
...
TextNodeCommon = __decorate([
    injectable()
], TextNodeCommon);
let TextNodeTitle = class TextNodeTitle {
    constructor() {
...
const blockSchema = new Schema({ nodes: nodes, marks: marks });
...
// To reuse elements from this schema, extend or read from its
// `spec.nodes` and `spec.marks` [properties](#model.Schema.spec).
const titleSchema = new Schema({ nodes: nodes$1, marks: marks$1 });
...
class DocumentTextNodeEditor extends LitElement {
    constructor() {
        super(...arguments);
        this.logger = new Logger('DOCUMENT-TEXT-NODE-EDITOR');
        this.editable = 'true';
    ...
    appendContent(content) {
        ...
        this.editor.view.dispatch(this.editor.view.state.tr.setSelection(TextSelection.near(this.editor.view.state.doc.resolve(end - 1))));
        this.editor.view.focus();
    }
    ...
    applyLinkMark() {
        ...
        if (href) {
            toggleMark(this.editor.view.state.schema.marks.link, { href })(this.editor.view.state, this.editor.view.dispatch);
            this.preventHide = false;
            this.selected = false;
        }
    }
...
class DocumentEditor extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
        this.logger = new Logger('DOCUMENT-EDITOR');
        this.checkOwner = false;
        this.readOnly = false;
        this.rootLevel = 0;
        this.parentId = '';
        this.defaultType = EveesModule.bindings.PerspectiveType;
        ...
        this.doc = undefined;
        this.draftService = new EveesDraftsLocal();
    }
    async firstUpdated() {
    ...
    hasChanges(node) {
        ...
        if (!node.data)
            return true;
        if (!isEqual(node.data.object, node.draft))
            return true;
        return false;
    }
    hasChangesRec(node) {
    ...
    async derivePerspective(node) {
        ...
        return deriveSecured(perspective, remoteInstance.store.cidConfig);
    }
...
class TextNodeDiff extends moduleConnect(LitElement) {
    constructor() {
        super(...arguments);
 ...
 */
class DocumentsModule extends EveesContentModule {
    constructor() {
        super(...arguments);
...
export { DocumentsBindings, DocumentsModule, TextNodeCommon, TextNodePattern, TextNodeTitle, TextType, htmlToText };
//# sourceMappingURL=uprtcl-documents.es5.js.map
